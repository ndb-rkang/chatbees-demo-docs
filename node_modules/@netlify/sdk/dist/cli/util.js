...
const getConfigurationFile = (pwd = process.cwd()) => {
    const fileName = possibleFiles.find((fileName) => existsSync(resolve(pwd, fileName)));
    return fileName;
};
export const getConfiguration = async (pwd = process.cwd()) => {
    const fileName = getConfigurationFile(pwd);
    if (!fileName) {
        throw new Error(`No configuration file found in ${pwd}`);
    }
    try {

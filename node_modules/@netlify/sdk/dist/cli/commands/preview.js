...
export const preview = async (opts) => {
    ...
    if (typeof opts.ui === `undefined` && !opts.connector) {
        opts.ui = true;
    }
    ...
    if (opts.connector) {
        return {
            startEngine: await handleConnector({ pwd, opts }),
        };
    }
    return {
...
const handleConnector = async ({ pwd, opts, }) => {
    const { builtPath } = await getIntegrationBuildInfo({
        directory: pwd,
    });
...
const handleUI = async ({ opts, pwd }) => {
    rmSync(resolve(pwd, ".ntli"), { recursive: true, force: true });
    const outDir = resolve(pwd, ".ntli");
    ...
    const stateFileExists = fsExtraPkg.existsSync(resolve(pwd, ".netlify/state.json"));
    const stateFile = stateFileExists
        ? JSON.parse(fsExtraPkg.readFileSync(resolve(pwd, ".netlify/state.json"), "utf8"))
        : {};
    ...
    if (buildResult) {
        ...
        devProcess.stdout.on("data", async (data) => {
            ...
            const { dev } = opts;
            if (/◈ Server now ready on/.test(output)) {
